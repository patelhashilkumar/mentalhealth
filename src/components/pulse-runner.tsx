...
const PulseRunner: React.FC = () => {
  ...
  useEffect(() => {
    ...
    function fit() {
      ...
      // Ensure it doesn't get too big on large screens, respect parent container
      const parent = canvas!.parentElement;
      if (parent) {
         const parentW = parent.clientWidth - margin*2;
         const parentH = parent.clientHeight- margin*2;
         scale = Math.min(scale, parentW / canvas.width, parentH / canvas.height);
      }
    ...
    function loop(t: number) {
      ...
      if (!gameOver && running) {
        ...
        if (pipes.length > 0 && pipes[0].x + PIPE_W < bird.x - bird.r && !pipes[0].passed) {
          ...
          pipes[0].passed = true; // Mark as passed
        }
